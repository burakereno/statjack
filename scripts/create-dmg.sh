...
APP_PATH="${1:-$ROOT_DIR/build/Release/StatJack.app}"
APP_BUNDLE_NAME="$(basename "$APP_PATH")"
OUTPUT_DMG="${2:-$ROOT_DIR/StatJack.dmg}"

VOLUME_NAME="${VOLUME_NAME:-StatJack}"
DMG_TITLE="${DMG_TITLE:-StatJack, built for}"
DMG_SUBTITLE="${DMG_SUBTITLE:-system stats at a glance}"
DMG_HINT="${DMG_HINT:-Drag StatJack to Applications}"
...
accent.lineWidth = 5
color(34, 197, 94).setStroke()
accent.move(to: NSPoint(x: gridXLeft, y: gridYLower))
accent.line(to: NSPoint(x: gridXRight, y: gridYLower))
accent.stroke()
...
let titleAttributes: [NSAttributedString.Key: Any] = [
    ...
    .foregroundColor: color(30, 41, 59),
...
let hintAttributes: [NSAttributedString.Key: Any] = [
    ...
    .foregroundColor: color(79, 70, 229),
    .paragraphStyle: paragraph
]
